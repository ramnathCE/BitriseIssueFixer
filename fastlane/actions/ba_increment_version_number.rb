module Fastlane
  module Actions
    ...
    class BaIncrementVersionNumberAction < Action
      require 'shellwords'
      ...
      def self.run(params)
        ...
        # added filters to ignore tests, widgets and watches
        current_version = `#{command_prefix} agvtool what-marketing-version -terse | grep -iv "Test" | grep -iv "Widgets" | grep -iv "Watch" | head -1 | sed 's/.*=//g'`.split("\n").last || ''
        ...
        command = [
          ...
          "&& agvtool new-version -all #{next_version_number.to_s.strip}"
        ].join(' ')
